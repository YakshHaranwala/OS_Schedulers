 ...
 * 
 * This file has the SJF schdeuler for the project. The purpose of the scheduler is 
 * to schedule the tasks based on Shortest Job First basis. Scheduling metrics like 
...
/* Array for burst time, exit time and start time for each process. */ 
int *burst_times;
int *exit_times;
int *start_times;
...
void add(char *taskName, int taskPriority, int burstTime){
    ...
    /* Allocating TID, burst time and the priority of the task and inserting it to the task list.*/
 ...
 * according to the SJF policy.
 * 
 * @return the task which has been picked.
 **/
Task *pickNextTask(){
    struct node *iter = task_list;
    Task *shortest = iter->task;
    while(iter != NULL){
        if(iter->task->burst < shortest->burst){
            shortest = iter->task;
        }
        iter = iter->next;
    }
