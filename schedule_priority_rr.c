...
Task *pickNextTask(){
    ...
    Task *highest = task_list->task;
    while(iter != NULL){
        if(highest->priority <= iter->task->priority){
            highest = iter->task;
        }
        iter = iter->next;
    }
...
void schedule(){
    ...
    /* Allocate the space for the required arrays. */
    burst_times = calloc(num_tasks, sizeof(int));
    exit_times = calloc(num_tasks, sizeof(int));
    start_times = calloc(num_tasks, sizeof(int));
    start_filled = calloc(num_tasks, sizeof(int));

    /* Fill the array of burst times for the tasks.*/
    struct node *iter = task_list;
    int i = 0;
    while(iter != NULL){
    ...
    /* Fill the flag array for start times to FALSE for each element.*/
    for(int i = 0; i < num_tasks; i++){
         start_filled[i] = FALSE;
    }
    
    /* Schedule the actual tasks and print their results from below.*/
    ...
    while(task_list != NULL){
        ...
        int process_num = atoi(&task->name[1]);
        if((start_times[process_num - 1] == 0) && (start_filled[process_num - 1] == FALSE)){
            start_times[process_num - 1] = cur_time;
            start_filled[process_num - 1] = TRUE;
        }    
...
/**
 * Reload the task with the same tid and the the new remaining burst time
 **/
